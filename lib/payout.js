...
function send(payoutData) {
  return new Promise(async function(resolve, reject) {
    try {
      ...
      if (payout.value > 0) {
        let spent = await wereAddressSpentFrom(payout.address, iotaNode)
        if (spent == true) {
...
function payoutValidator({
  ...
  return new Promise(async function(resolve, reject) {
    try {
      ...
      try {
        //check last trit for Kerl address
        if (checksum.isValidChecksum(address) && !/[E-V]/.test(address.slice(80, 81))) {
