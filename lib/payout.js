...
function sendPayout(payout) {
  return new Promise(async function(resolve, reject) {
    try {
      ...
      if (value < totalBalance) {
        ...
        for (openPayout of openPayouts) {
          if (openPayout.id != id) {
            ...
            //limit one bundle to max 10 outgoing value txs
            if (additionalPayouts.length > 7) {
              break
            }
          }
        }
      ...
      await store_raw_bundle(preparedBundle[0].bundle, trytes)

      //if the function crashes after the raw txs are store and the payout isn't updated it will get payed but not updated
      ...
      let addressIndexesForPayouts = await get_indexes_for_payouts()
      ...
      for (index of addressIndexesForPayouts) {
        if (usedIndexes.indexOf(index) != -1) {
          await delete_index_for_payouts(index)
        }
      }

      //update spentAddresses
      for (address of spentAddresses) {
        await update_spent_address(address)
      }
...
function updatePayout(payout) {
  return new Promise(async function(resolve, reject) {
    let updated_payoutObject = await update_payout(
      { id: payout.id },
...
function getOpenPayouts() {
  return new Promise(async function(resolve, reject) {
    let open_payouts = await get_open_payouts()
...
function getPayoutByID(id) {
  return new Promise(async function(resolve, reject) {
    const Payout = await get_payout({ id: id })
    resolve(Payout)
  })
}
...
function getPayoutByTxhash(txhash) {
  return new Promise(async function(resolve, reject) {
    const Payout = await get_payout({ txhash: txhash })
    resolve(Payout)
  })
}
...
async function searchInputAdresses(value, startIndex, endIndex) {
  ...
  let addressIndexesForPayouts = await get_indexes_for_payouts()
  let allAddressesDb = await get_addresses()

  for (index of addressIndexesForPayouts) {
    let addressObject = allAddressesDb.filter(a => a.index == index)
