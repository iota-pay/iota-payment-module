...
function sendPayoutBundle(payout) {
  return new Promise(async function(resolve, reject) {
    try {
      ...
      if (value < totalBalance) {
        ...
        if (value < totalBalance) {
          ...
          if (config.debug === 'basic' || config.debug === 'full') {
            ...
            console.log('ransfers.length+2', transfers.length + 2)
            console.log(
              'addressesWithBalanceAndIndex.length * 2+transfers.length+2',
              addressesWithBalanceAndIndex.length * 2 + transfers.length + 2
            )
            console.log('config.maxBundleSize', config.maxBundleSize)
          }
          if (
            addressesWithBalanceAndIndex.length * 2 + transfers.length + 2 <=
              config.maxBundleSize &&
      ...
      if (typeof remainderAddressIndexes != 'undefined') {
        rawBundleObject.remainderAddressIndexes = remainderAddressIndexes
      }
      await storeRawBundle(rawBundleObject)
