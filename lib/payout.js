...
function sendPayoutBundle(payout) {
  return new Promise(async function (resolve, reject) {
    try {
      ...
      //interal transfers are transactions to own addresses to merge funds from multiple inputs, so a bundle never exceeds the maxBundleSize
      if (
        (totalBalance > value &&
        ...
        let internalTransferAddress = generate_address(
          SEED,
          newAddressIndex,
          2,
          true
        ...
        let preparedBundle = trytes.map((e) =>
          txconverter.asTransactionObject(e)
        )
        ...
        let rawBundleObject = {
          ...
          remainingValueforBalance: internalTransferValue,
        }
      ...
      let remainingValueforBalance = 0
      if (value < totalBalance) {
        //search and add additional payouts without searching for new inputs
        let openPayouts = await getOpenPayouts()
        let sortedOpenPayouts = openPayouts.sort((a, b) => b.value - a.value)
        let additionalPayouts = []
        for (openPayout of sortedOpenPayouts) {
          //get amount of open payouts
          let lengthOpenPayouts =
            sortedOpenPayouts.length - additionalPayouts.length
          //break with one transfer less to add a second remainder
          if (sortedOpenPayouts.length > 4)
            if (
              ...
              (totalBalance - value) / 2 >
                sortedOpenPayouts[sortedOpenPayouts.length - 4].value
            ) {
              if (config.debug === 'basic' || config.debug === 'full') {
                console.log(
        ...
        if (value < totalBalance) {
          //set remaining value which will be used for getBalance call
          remainingValueforBalance = totalBalance - value
          //get amount of open payouts
          ...
          if (
            addressesWithBalanceAndIndex.length * 2 + transfers.length + 2 <=
              config.maxBundleSize &&
            lengthOpenPayouts > config.maxBundleSize &&
            //check if it makes sense to split the funds even more, has to be > than the third lowest value, /2 because of two remainder addresses
            (totalBalance - value) / 2 >
              sortedOpenPayouts[sortedOpenPayouts.length - 4].value
          ) {
            ...
            let remainderAddressOne = generate_address(
              SEED,
            ...
            let remainderAddressTwo = generate_address(
              SEED,
            ...
            let remainderAddress = generate_address(
              SEED,
      ...
      let rawBundleObject = {
        ...
        remainingValueforBalance,
      }
      await storeRawBundle(rawBundleObject)

      //if the function crashes after the raw txs are store and the payouts aren't updated they will get paid but not updated
      payoutInfo.forEach((e) => (e.txhash = preparedBundle[0].bundle))
      for (payoutdata of payoutInfo) {
        await updatePayout(
          { id: payoutdata.id },
          { txhash: payoutdata.txhash, paid: 'pending' }
        )
      }
...
async function getAddressBalances(addresses) {
  let balanceResponse = await iota.getBalances(addresses)
...
function payoutValidator({
  ...
  return new Promise(async function (resolve, reject) {
    try {
      ...
      //check if address is from own seed
      if (payout.value > 0) {
        let ownAddressObjects = await getAddresses()
        let ownAddresses = ownAddressObjects.map((e) => e.address)
        if (ownAddresses.indexOf(address) != -1) {
          throw 'Address is from own seed'
        }
      }
      ...
      if (typeof message != 'undefined' && message != '') {
        try {
          converter.asciiToTrytes(message)
        } catch (e) {
          throw 'Invalid ascii charactes. Extended ascii is not supported.'
        }
        if (message.length > config.maxBundleSize * 1093) {
          throw 'Message is too long, increase maxBundleSize'
        }
...
function wereAddressSpentFrom(address) {
  return new Promise(async (resolve, reject) => {
    try {
      const post = bent(config.iotaNodes[0], 'POST', 'json', {
