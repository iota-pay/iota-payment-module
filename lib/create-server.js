...
const { initialize_mongodb } = require('./Database.js')
...
async function start_handlers() {
  try {
    ...
    if (process.env.db == 'mongodb') {
      await initialize_mongodb().catch(err => {
...
function createServer(app, options = {}) {
  ...
  if (options.fast_but_risky === 'true' || options.zmq === 'true') {
    if (typeof options.zmq_node === 'undefined') {
      throw 'Undefined zmq node, pls add one in the config params or disable zmq/fast_but_risky'
    } else {
      startZmq()
