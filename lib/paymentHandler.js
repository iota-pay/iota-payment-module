...
function checkForTransactions(payments) {
  //look for incoming value transactions
  let paymentsToCheck = payments.filter(e => e.earlyAccepted != true)
  let paymentAdresses = paymentsToCheck.map(e => e.address)
  iota
    ...
    .then(transactions => {
      transactions.forEach(tx => {
        if (tx.value > 0) {
          ...
          console.log(
            `checkForTransactions(): Incoming transaction for payment ${payment.id}: value tx: ${tx.value} value payment: ${payment.value}`
          )
          if (tx.value >= payment.value) {
            const message = tx.signatureMessageFragment
            const txInfo = {
              message: Converter.trytesToAscii(message + "9").replace(/\0/g, ''),
