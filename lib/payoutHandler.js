...
function start() {
  ...
  //start immediately
  checkpayoutstatus()
  let intervall = setInterval(checkpayoutstatus, 20000)
  payoutHandler = true
}
...
let reattach = false
checkSentPayouts()
setInterval(() => checkSentPayouts(), 20000)
    ...
    for (rawBundle of rawBundles) {
      try {
        ...
        let allTailTxsofBundle = bundleObjects
          ...
          .map((tx) => tx.hash)
        let inclusionStates = await iota.getInclusionStates(allTailTxsofBundle)
        //if unconfirmed
