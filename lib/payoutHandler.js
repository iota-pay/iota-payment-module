...
async function checkSentPayouts() {
  try {
    ...
    for (bundleHash of bundles) {
      let bundleObjects = await iota.findTransactionObjects({ bundles: [bundleHash] })
      let allTailTxsofBundle = bundleObjects.filter(tx => tx.currentIndex == 0).map(tx => tx.hash);
      ...
      if (inclusionStates.indexOf(true) === -1) {
        ...
        await reattachAndPromote(pendingPayoutTxs[bundleHash])
      } else {
        //delete confirmed trytes
        db.get('pendingPayoutTxs').unset([bundleHash]).write()
      }
    }
  } catch (e) {
    console.error(e);
