        ...
        if (sendStatus == false) {
          ...
          sendPayoutBundle(payouts[0])
            .then(async payed => {
              ...
              if (config.debug == 'basic' || config.debug == 'full') {
                console.log(
                  'payout sent: ' + config.explorerTxLink + payed.txhash
                )
              }
              sendStatus = false
      ...
      try {
        ...
        if (inclusionStates.indexOf(true) === -1) {
          ...
          for (payout of rawBundle.payouts) {
            if (payout.info != 'internalTransfer') {
              let newPayout = await updatePayout(
                { id: payout.id },
                { txhash: payout.txhash, payed: true }
              )
          ...
          //store remainder address so it's available as input
          if (typeof rawBundle.remainderAddressIndex != 'undefined') {
            await storeIndexForPayouts(rawBundle.remainderAddressIndex)
          }
