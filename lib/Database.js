...
if (process.env.db == 'mongodb') {
  ...
  if(JSON.stringify(lowdb.value())==JSON.stringify(initial_state)){
    console.log(`First address (index 0): ${initial_state.addresses[0].address} can be used to deposit intial funds`);
  }
}
...
function store_address(address_object) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
...
function update_spent_address(address) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function update_payment(id, updated_fields) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function update_payout(id, updated_fields) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function increase_keyIndex() {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
        let index = await get_keyIndex()
...
function set_payoutIndex(new_payoutIndex) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function delete_raw_bundle(bundlehash) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function store_raw_bundle(bundlehash, bundle_trytes) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
        let testIfEmpty = await get_raw_bundles()
        if (testIfEmpty == null) {
          //insert if empty
          await db
            .collection('rawBundles')
            .insertOne({ [bundlehash]: bundle_trytes })
        } else {
          await db
            .collection('rawBundles')
            .updateOne({}, { $set: { [bundlehash]: bundle_trytes } })
        }

        resolve()
      } else {
        lowdb.set(`rawBundles.${bundlehash}`, bundle_trytes).write()
        resolve()
      }
    } catch (err) {
      reject(err)
...
function store_index_for_payouts(index_for_payouts) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function get_indexes_for_payouts() {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
function delete_index_for_payouts(index_for_payouts) {
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
...
module.exports = {
  ...
  get_indexes_for_payouts,
  delete_index_for_payouts
}
