...
let seed_checksum = checksum.addChecksum(config.seed, 3, false).slice(-3)
const filename =
  'db_iota-payment-' + config.seed.substring(0, 3) + seed_checksum + '.json'
let uri = config.mongodbUrl + config.seed.substring(0, 3) + seed_checksum
...
let initial_state = {
  ...
  addresses: [
    {
      address: iotaCore.generateAddress(config.seed, 0, 2, true),
      index: 0,
    },
    {
      address: iotaCore.generateAddress(config.seed, 1, 2, true),
      index: 1,
    },
  ],
...
if (config.db == 'mongodb') {
  ...
  if (config.debug === 'basic' || config.debug === 'full') {
    console.log('lowdb is used with ' + filename)
  }
  db_type = 'low'
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        lowdb.get('addresses').push(address_object).write()
        resolve()
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        lowdb.get('payments').push(payment).write()
        resolve()
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        lowdb.get('payments').find({ id }).unset(updated_fields).write()
        let updated_payment = lowdb.get('payments').find({ id }).value()
        resolve(updated_payment)
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let deletedBundle = lowdb.get('payments').remove({ id }).write()
        resolve(deletedBundle)
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let payment_objects = lowdb.get('payments').find(key).value()
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let address_objects = await db
          .collection('payments')
          .find({ paid: false }, { projection: { _id: 0 } })
          .toArray()
        resolve(address_objects)
      } else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let address_objects = await db
          .collection('payments')
          .find({ paid: true }, { projection: { _id: 0 } })
          .toArray()
        resolve(address_objects)
      } else {
        let address_objects = lowdb
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        lowdb.get('payouts').push(payout).write()
        resolve()
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let deletedBundle = lowdb.get('payouts').remove({ id }).write()
        resolve(deletedBundle)
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        let payout_objects = lowdb.get('payouts').find(key).value()
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        lowdb.get('rawBundles').push(bundleobject).write()
        resolve()
      }
    } catch (err) {
      reject(err)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        let db = await connect_to_mongodb()
        await db
          .collection('indexesForPayouts')
          .update({}, { $push: { indexesForPayouts: index_for_payouts } })
        resolve()
      } else {
        lowdb.get('indexesForPayouts').push(index_for_payouts).write()
        resolve()
      }
    } catch (err) {
      reject(err)
      ...
      if (db_type == 'mongo') {
        ...
        let index_object = await db
          ...
          .find(
            {},
            {
              $pull: { indexesForPayouts: 'indexesForPayouts' },
              projection: { _id: 0 },
            }
          )
          .toArray()
        if (index_object.length == 0) {
          index_object[0] = { indexesForPayouts: [] }
        }
        resolve(index_object[0].indexesForPayouts)
      } else {
        let index_object = lowdb.get('indexesForPayouts').value()
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (db_type == 'mongo') {
        ...
        await db
          .collection('indexesForPayouts')
          .update({}, { $pull: { indexesForPayouts: index_for_payouts } })
        resolve()
      } else {
        lowdb.get('indexesForPayouts').pull(index_for_payouts).write()
        resolve()
      }
    } catch (err) {
      reject(err)
